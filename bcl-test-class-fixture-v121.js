...
const VERSION='v122-stable-fixture';
...
function makeSkill(i){
  ...
  return {
    id:`TST-${spec==='Common'?'C':'A'}-${String(i%50+1).padStart(3,'0')}`,name:names[i],spec,category:spec,role,input,inputs:[input],
    duration,executionSeconds:duration,timingSeconds:duration,timer:duration,damage,hits,dps:Number((damage/duration).toFixed(2)),pveDamage:String(damage),
    pvpDamage:hasPvp?String(pvpDamage):'',pvpDamagePercent:hasPvp?pvpDamage:null,staminaCost,resourceDelta,cooldown,accuracy,critRate,recovery,
    cc,ccPoints:ccPoints(cc),ccTimerSeconds:ccTimer,ccDuration:ccTimer,protection,protections:pLabel?[pLabel]:[],
    ...
    downAttack,airAttack,backAttack,downSmash,airSmash,mobility:rnd()<.35?r(1,10,2):0,range:rnd()<.50?r(.8,16,1):null,aoe:rnd()<.44?r(1,10,2):null,
...
function expose(){globalThis.BCL_TEST_CLASS_V122={version:VERSION,name:CLASS,synthetic:true,skills:allSkills,common,awakening,transitions,transitionChains,counts:{skills:100,common:50,awakening:50,cc:20,pvpProfiled:60,transitions:20,cancels:20,chains:3}}}
function scheduleSingleRefresh(){
  setTimeout(()=>{
    try{if(typeof window.bclScheduleLiveRefresh==='function'){window.bclScheduleLiveRefresh();return}}catch(e){}
...
function install(){if(!ensureOption())return false;resetStores();expose();return true}
function boot(){
  install();
  ...
  if(sel){
    sel.addEventListener('change',e=>{
      if(e.target.value!==CLASS)return;
      resetStores();expose();scheduleSingleRefresh();
    },true);
  }
  document.addEventListener('bcl-cloud-sync-complete',()=>{
    const active=document.getElementById('classSelector')?.value===CLASS;
    if(active){resetStores();expose();}
  });
}
