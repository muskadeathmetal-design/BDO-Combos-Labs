...
function mergeSkills(existing,staticSkills){const out=Array.isArray(existing)?existing.map(clone):[];const by=new Map(out.map((s,i)=>[norm(s?.name),i]));for(const raw of staticSkills){const s=clone(raw),k=norm(s.name);if(!k)continue;if(by.has(k)){const i=by.get(k);out[i]={...s,...out[i]};}else{out.push(s);by.set(k,out.length-1);}}return out;}
function transitionKey(t){return `${norm(t?.from||t?.fromSkill)}>${norm(t?.to||t?.toSkill)}`;}
function mergeTransitions(existing,staticTransitions){const out=Array.isArray(existing)?existing.map(clone):[];const by=new Map(out.map((t,i)=>[transitionKey(t),i]));for(const raw of staticTransitions){const t=clone(raw),k=transitionKey(t);if(!k||k==='>')continue;if(by.has(k)){const i=by.get(k);out[i]={...t,...out[i]};}else{out.push(t);by.set(k,out.length-1);}}return out;}
const state=read();
state.version=3;
...
const alreadyLoaded=state?.staticData?.manualV312Loaded===true;
const dataComplete=skillTarget.Awakening.length>=90&&trTarget.Awakening.length>=20;
if(alreadyLoaded&&dataComplete){console.info('[V3.12] Static combat data already installed');return;}
...
save(state);
console.info('[V3.12] Static combat data installed',{awakeningSkills:skillTarget.Awakening.length,successionSkills:skillTarget.Succession.length,awakeningTransitions:trTarget.Awakening.length,successionTransitions:trTarget.Succession.length});
})();